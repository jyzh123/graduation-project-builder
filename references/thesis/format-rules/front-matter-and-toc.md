...
- Cover media, icons, school marks, logos, and other template-owned cover drawings are part of `cover_style` only when the active official template or approved sample actually owns such a cover-zone drawing. If the template cover has media, the final DOCX must prove that the cover drawing/object exists in the cover zone, keeps a DOCX relationship id, and resolves to an existing `word/media/*` package target. If the template cover is text-only, the final DOCX must record a template no-cover-media baseline and must not insert a fake logo, placeholder, or body figure into the cover just to satisfy a media-count gate. A cover screenshot, visible image crop, or count-only media inventory is not enough.
- Cover and front-matter hard fields must be explicit, not inferred from body style. The evidence must expose paragraph style id, alignment, indentation, spacing, tab stops, run font slots, run font size, and style binding for the cover title/identity rows, declaration/title-front-matter rows, Chinese abstract, English abstract, and TOC title/entries as applicable.
...
- A body-heading insertion pass must prove that its target anchor is after the real first body chapter and outside the live/static TOC field result. A `Heading1`, `Heading2`, `Heading3`, or `Heading4` paragraph between the TOC title and the first body section boundary is TOC corruption unless it is a real TOC entry with the correct TOC style, right dotted tab leader, and visible page number.
- Never verify "uses三级标题" by counting `Heading3` paragraphs globally. The count must exclude cover/front matter, TOC/cache rows, captions, references, acknowledgements, appendices unless those surfaces are explicitly in scope, and any heading-style paragraph found inside the TOC block fails TOC acceptance.
- TOC acceptance must include bidirectional coverage: every real body level-1, level-2, and level-3 heading in the body range must have a matching visible TOC cache row, and every matched row must carry visible page-number text plus right dotted-tab semantics. A live TOC field, existing level-1 rows, or a global `Heading3` count cannot substitute for missing level-3 cache rows.
- Static TOC page-number synchronization may update existing TOC page-number runs only after the mapped body-heading list is proven present in the TOC cache. If any mapped level-1/2/3 body heading is absent from the TOC block, rebuild the TOC cache first instead of silently updating the rows that happen to exist.
...
- TOC title style binding is a hard surface in its own right. A target TOC title that visually says `目录` but uses `Normal`, body, heading, or any non-template style while the active template has a distinct TOC title paragraph style is failing. `scripts/measure_toc_paragraph_typography.py` must compare the template and target TOC title style id/name and fail on title style-binding mismatch; entry-level TOC metrics cannot substitute for the TOC title verdict.
- TOC title format judgement must include title text compactness, absence of tab/page-number runs, paragraph alignment, and page-break-before state. A title paragraph that says `目录` but is left/default aligned, contains a tab, carries page-number residue, or is not compactly the template TOC title remains failing even when TOC entries and dotted leaders pass. If the active manuscript needs a TOC title page break to separate English abstract material from the TOC, preserving that page break is valid even when a converted template/specification donor does not expose the same flag.
...
- TOC page-number column alignment is a structural tab-stop surface, not only a visual page-number surface. For every visible TOC entry, evidence must prove a right-tab stop or equivalent template-owned right-alignment semantics before the page-number run, the dotted leader owner, and the page-number column x position. A TOC with correct page-number text but left/default tab stops, missing right-tab metadata, or only prose saying "right aligned" fails.
...
- First-body rendered-page detection must skip every rendered TOC continuation page before searching for the body opener. A TOC row such as `第1章 绪论 ... 1` is not proof of the body opener page. The detector must use the visible TOC entries to identify the TOC span, then search after that span and allow spacing-normalized heading matching for renderers that output `第1 章绪论` or similar extraction spacing.
- Template-profile learning from an official writing/specification document must fail closed on missing real cover or abstract markers instead of borrowing nearby examples. Reference examples, drawing-package notes, bibliography entries, and formatting instructions are not valid cover-title, abstract, or first-body markers; mark those surfaces as absent and require a separate approved baseline profile or final-output evidence.
...
- Tab-stop fidelity rule:
  ...
  - validator-backed evidence must include `TOC right-tab stop semantic baseline/actual`, `TOC page-number column right alignment baseline/actual`, `TOC page-number tab leader ownership baseline/actual`, and `TOC per-entry right-tab/page-number verdict`
...
- TOC title font and every visible TOC entry font are hard audit fields. Sample-self-check or rendered geometry reconciliation must not suppress TOC title-font, entry-font, or visible-run typography failures.

- Rendered page-class detectors must tolerate extraction-only bullet or symbol prefixes from Word/WPS PDF export, such as `•目录` or `•第1章 绪论`. Strip those prefixes before comparing standalone headings, and when the DOCX marker is only the chapter prefix such as `第1章`, allow a rendered standalone line that starts with that prefix. Do not relax the requirement that TOC and first body heading appear as their own rendered lines on the correct pages.
- Rendered TOC page detection must not treat a running header/footer string such as `目录` on Chinese abstract, English abstract, or TOC-continuation pages as the TOC title page by itself. A rendered page is the TOC start only when the title marker is accompanied by real TOC evidence: multiple TOC entries from the final DOCX, visible dotted/leader rows with page numbers, a page-number column, or equivalent live-TOC field output. The detector must also recognize ordinary period leaders such as `标题........ 12`, not only Unicode ellipsis leaders.
- Page-marker learning must never let a TOC title paragraph double as `first_body`, even if the TOC title carries a heading-like style in a converted school template. Once a paragraph is identified as the TOC marker, skip first-body marker detection on that same paragraph and search for the next real body chapter heading.
